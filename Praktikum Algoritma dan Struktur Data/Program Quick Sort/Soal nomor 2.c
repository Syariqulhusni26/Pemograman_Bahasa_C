...
// Fungsi untuk memilih pivot sebagai elemen terakhir dan mengatur elemen-elemen yang lebih besar daripadanya ke sebelah kiri dan yang lebih kecil ke sebelah kanan
int partition(int Nrendah, int Ntinggi)
{
    ...
    {
        if (a[j] > pivot) // Perubahan: perbandingan ">" untuk descending
        {
            i++;
            swap(&a[i], &a[j]);
        }
